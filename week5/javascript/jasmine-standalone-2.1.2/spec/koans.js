...
describe("common patterns", function(){
   it("revealing pattern", function(){
        
        var myNamespace = {};        

        //OJO CON LOS PARENTESIS ALREDEDOR DE LA FUNCION!!!
        (function(ns){
           var counter = 0;
           function privateMethod(){
               /*...*/
           }
           ns.currentCount = function(){
               return counter;
           };
           ...
           ns = {}; //este objeto no influye en nada
        }(myNamespace));        

        myNamespace.increment();
        myNamespace.increment();
        myNamespace.decrement();
        expect(myNamespace.currentCount()).toBe(1);
   });

    it("considers objects as references",function(){
        function f(arg){
            arg.x = 1;
        }
        var obj = {x: 0};
...
describe("the JavaScript language", function(){
  ...
  describe("has different types and operators that", function(){
    ...
    it("joins parts as string when using the plus operator", function(){
        expect(1 + "a" + 4).toEqual("1a4");
    });


    //SOLO SUMA ANTES DEL PRIMER "STRING"
      

    it("operates integers before joining the string", function(){
        expect(1 + 1 + "2" + 3 + 3 + "2").toEqual('22332');
    });


    //UNA VARIABLE INICIALIZADA SIN VALOR ES "UNDEFINED"
    ...
    it("knows the type of the variable", function(){
        ...
        expect(typeof(y)).toEqual('undefined');
        expect(typeof(z)).toEqual('boolean');
        expect(typeof(str)).toEqual('string');
    });

    //WTF ES DE TIPO NUMERO (INTEGER)
    //CUALQUIER CALCULO CON UNDEFINED ES "WTF"

    it("surprises me, NaN is not comparable with NaN", function(){
        var und;
        //expect(5 / "a").not.toEqual(5 / "a");
        expect(typeof(NaN)).toEqual('number');
        expect(typeof(5 / "a")).toEqual('number');
        expect(typeof(true / true)).toEqual('number');

        expect(isNaN(5 / "a")).toBeTruthy();
        expect(isNaN(false / false)).toBeTruthy();
       // expect(isNaN(true / true)).toBeTruthy();
        expect(isNaN(und * 1)).toBeTruthy();
    ...
    // SOLO SE CONCATENAN ARRAYS SI PONEMOS UNA CADENA VACÍA
    // SIN LA CADENA VACÍA SERÍA [1,2,34,5,6]
